...
class SimpleMIDTI(nn.Module):
    """
    A practical "MIDTI-like" graph branch:
      - build 4 graphs: dd, pp, dp, ddpp
      - GCN on each (3 layers)
      - stack multiple views then DIA attention blocks
      - output 1 logit per (drug,target) pair
    """
    def __init__(self, n_drug, n_prot, dim=256, n_heads=8, dia_layers=2, dropout=0.1, mlp_hidden=128):
        ...
        self.g_dp = GCNStack(dim)     # on joint
        self.g_ddpp = GCNStack(dim)   # on joint

        self.dia = nn.ModuleList([DIA_Block(dim, n_heads, dropout) for _ in range(dia_layers)])
        self.dia_layers = dia_layers

        # compress concatenated outputs
        # we will concatenate original + each DIA output -> (dia_layers+1) * dim
    ...
    def forward(self, graphs, feat_drug, feat_prot, drug_idx, prot_idx):
        ...
        feat_joint = torch.cat([feat_drug, feat_prot], dim=0)  # (nD+nP,dim)

        # GCN outputs
        dd1, dd2, dd3 = self.g_dd(graphs["dd"], feat_drug)              # (nD,dim)
        pp1, pp2, pp3 = self.g_pp(graphs["pp"], feat_prot)              # (nP,dim)
        dp1, dp2, dp3 = self.g_dp(graphs["dp"], feat_joint)             # (nD+nP,dim)
        hp1, hp2, hp3 = self.g_ddpp(graphs["ddpp"], feat_joint)         # (nD+nP,dim)
        ...
        # stack views: dd(3) + dpD(3) + hpD(3) => 9 views
        drug_stack = torch.stack([dd1, dd2, dd3, dpD[0], dpD[1], dpD[2], hpD[0], hpD[1], hpD[2]], dim=1)  # (nD,9,dim)
        prot_stack = torch.stack([pp1, pp2, pp3, dpP[0], dpP[1], dpP[2], hpP[0], hpP[1], hpP[2]], dim=1)  # (nP,9,dim)

        drugs = drug_stack[drug_idx]  # (B,9,dim)
        prots = prot_stack[prot_idx]  # (B,9,dim)

        # DIA blocks, collect outputs
        d_cat = drugs
        p_cat = prots
        for blk in self.dia:
